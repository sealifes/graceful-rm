...
if [[ -f "$HERE/scripts/install-hooks.sh" ]]; then
  exec "$HERE/scripts/install-hooks.sh" "$@"
fi
...
if command -v curl >/dev/null 2>&1; then
  ...
  echo "install-hooks.sh: curl or wget is required for remote installation" >&2
  exit 1
fi

tar -xzf "$archive" -C "$tmp_dir"
source_dir="$(find "$tmp_dir" -mindepth 1 -maxdepth 1 -type d -name 'graceful-rm-*' -print -quit)"
exec "$source_dir/scripts/install-hooks.sh" "$@"
